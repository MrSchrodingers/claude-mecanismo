...
# herdarem EVIDENCE_GATE_LOCK=held, pularem a aquisicao, e o teste passaria sem exercitar nada
# - a forma vacua que este repositorio ja pagou cinco vezes. Por isso todo filho e lancado com
# `env -u EVIDENCE_GATE_LOCK`, e o caso C4 existe justamente para provar que a marca foi
...
export EVIDENCE_GATE_LOCK_FILE="$T/suite-sob-teste.lock"
...
env -u EVIDENCE_GATE_LOCK bash "$T/cliente.sh" 5 >"$T/a.out" 2>"$T/a.err" &
BG=$!
...
env -u EVIDENCE_GATE_LOCK bash "$T/cliente.sh" >"$T/b.out" 2>"$T/b.err"; RC2=$?
...
env -u EVIDENCE_GATE_LOCK bash "$T/cliente.sh" >"$T/c.out" 2>&1; RC3=$?
...
env -u EVIDENCE_GATE_LOCK bash "$T/cliente.sh" 5 >"$T/d.out" 2>&1 &
BG2=$!
for _ in $(seq 1 100); do grep -q adquirido "$T/d.out" 2>/dev/null && break; sleep 0.1; done
EVIDENCE_GATE_LOCK=held bash "$T/cliente.sh" >"$T/e.out" 2>&1; RC4=$?
...
RC5=$(env -u EVIDENCE_GATE_LOCK bash -c 'echo "${EVIDENCE_GATE_LOCK:-VAZIO}"')
